...
def validate_and_convert(string_data):
    ...
    if not all(key in string_data for key in ['deviceid', 'name', 'ammonia', 'ph', 'temperature']):
    ...
    try:
        ...
        if not all(key in dictionary_data for key in ['deviceid', 'name', 'ammonia', 'ph', 'temperature']):
        ...
        if not isinstance(dictionary_data['deviceid'], str) or not isinstance(dictionary_data['name'], str) or not is_float(dictionary_data['ammonia']) or not is_float(dictionary_data['ph']) or not is_float(dictionary_data['temperature']):
